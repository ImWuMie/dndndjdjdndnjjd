...
public class TextLayoutEngine extends FontResourceManager
     ...
     */
    private volatile int mResLevel = 1;
    /**
     * Text direction.
     */
    ...
    public void reload() {
        ...
        mFontCollections.putIfAbsent(Identifier.of(SANS_SERIF.getPath()),
            Typeface.SANS_SERIF);
        mFontCollections.putIfAbsent(Identifier.of(SERIF.getPath()),
            Typeface.SERIF);
        mFontCollections.putIfAbsent(Identifier.of(MONOSPACED.getPath()),
            Typeface.MONOSPACED);
    ...
    public void onFontRegistered(@Nonnull FontFamily f) {
        ...
        try {
            ...
            if (mRegisteredFonts.putIfAbsent(location, fc) == null) {
                ...
                mRegisteredFonts.putIfAbsent(Identifier.of(newName), fc);
            }
        } catch (Exception e) {
            ...
            for (var resource : entry.getValue()) {
                try (var reader = resource.getReader()) {
                    ...
                    for (int i = 0; i < providers.size(); i++) {
                        ...
                        var definition = FontLoader.Provider.CODEC.parse(
                            JsonOps.INSTANCE,
                            metadata
                        ).getOrThrow().definition();
                        loadSingleFont(resources, name, bundle,
                            resource.getPackId(), i, metadata, definition);
                    }
                    LOGGER.info(MARKER, "Loaded raw font '{}' in pack: '{}'",
                        name, resource.getPackId());
                } catch (Exception e) {
                    LOGGER.warn(MARKER, "Failed to load font '{}' in pack: '{}'",
                        name, resource.getPackId(), e);
                }
            }
        ...
        if (parent != null) {
            ...
            families.removeIf(f -> {
                ...
                if (font instanceof BitmapFont b && b.mFontSize != fontSize) {
                    return true;
                } else if (font instanceof SpaceFont s && s.mFontSize != fontSize) {
                    return true;
                }
                return false;
            });
