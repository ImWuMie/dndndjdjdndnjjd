...
public class Vector3dSetting extends Setting<Vector3d> {
    ...
    protected JsonObject save(JsonObject tag) {
        JsonObject valueTag = new JsonObject();
        valueTag.addProperty("x", get().x);
        valueTag.addProperty("y", get().y);
        valueTag.addProperty("z", get().z);
    ...
    @Override
    protected Vector3d load(JsonObject tag) {
        JsonObject valueTag = tag.getAsJsonObject("value");

        set(valueTag.get("x").getAsDouble(), valueTag.get("y").getAsDouble(), valueTag.get("z").getAsDouble());
